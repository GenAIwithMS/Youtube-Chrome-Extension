...
let socket = null;
let setProgress = () => {}; 
const clientId = Date.now(); // A simple unique ID for this session
...
document.addEventListener('DOMContentLoaded', async () => {
  // --- START: Progress Ring Initialization ---
  const circle = document.querySelector('.progress-ring__circle');
  if (circle) { // Only run if the circle element exists
    ...
    // Now, redefine the global setProgress function to do the real work
    setProgress = function(percent) {
        const offset = circumference - (percent * circumference);
        circle.style.strokeDashoffset = offset;
    }
  }
  // --- END: Progress Ring Initialization ---
